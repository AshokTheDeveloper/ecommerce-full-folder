...
class AppStore extends Component {
  ...
  getCategoryApplications = () => {
    ...
    const apps = appsList.filter(eachApp => eachApp.category === activeTabId)
    return apps
  }

  render() {
    const {activeTabId} = this.state
    const getCategoryApps = this.getCategoryApplications()
    const {inputSearch} = this.state
    const filteredAppsList = getCategoryApps.filter(eachApp =>
