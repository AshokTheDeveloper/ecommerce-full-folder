...
class Gallery extends Component {
  state = {
    previewImage:
      'https://assets.ccbp.in/frontend/react-js/nature-mountain-with-pond-img.png',
    alterText: 'nature mountain with pond',
  }

  onClickChangeImage = (imageUrl, imageAltText) => {
    this.setState({
      previewImage: imageUrl,
      alterText: imageAltText,
    })
  }

  render() {
    const {previewImage, alterText} = this.state
    return (
      <div className="bg-container">
        <div className="gallery-container">
          <img src={previewImage} className="gallery-image" alt={alterText} />
          ...
          <ul className="thumbnail-bg-container">
            {imagesList.map(eachImageDetails => (
              <ThumbnailItem
                ...
                isTrue={eachImageDetails.imageAltText === alterText}
              />
            ))}
          </ul>
        </div>
